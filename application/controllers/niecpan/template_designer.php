...
class Template_designer extends CI_Controller {
	...
	public function save_xml_template()
	{
		...
		if($design_lock_status == 2) /* Template is unlocked */
		{
			...
			/* save publish log in template_log_master */
			if($publish_xml == "publish")
			{
			$previous_published_version = $page_details['Published_Version_Id'];
			$version_detail = $this->template_design_model->template_rendering_by_version_id($previous_published_version);	
			$before_update = $page_details;
			$this->template_design_model->save_template_publish_log($pageId,$before_update,$version_detail);
			}
