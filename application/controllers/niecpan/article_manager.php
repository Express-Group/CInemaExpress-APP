...

{
...

	{
		...
		if (check_article_id($article_id) != 0)
		{
			...
			*/
			...
			$data['review']=$this->article_model->get_review($article_id);
...

	{
		...
		if($publishdate != '') 
		$data['publishdate']    	= date('dS  F Y',strtotime($publishdate));
		else 
		$data['publishdate']    	= date('dS  F Y');	
		
		if($last_update != '')
		$data['last_update']    	= date('dS  F Y',strtotime($last_update));
		else 
		$data['last_update']    	=  date('dS  F Y');	
	...
	public function share(){
		$return = [];
		$return['status'] = 0;
		$return['message'] = 'Invalid arguments passed';
		$contentId = $this->input->post('id');
		$type = $this->input->post('type');
		$tbl = $this->input->post('tbl');
		if($contentId!=''){
			$CI = & get_instance();
			$live_db = $CI->load->database('live_db' , TRUE);
			$archive_db = $CI->load->database('archive_db' , TRUE);
			$checkId = $CI->db->select('sid')->from('socilamedia_master')->where(['content_id' => $contentId , 'type' => $type])->get()->num_rows();
			if($checkId==0){
				if($tbl!=''){
					
				}else{
					$details = $live_db->select('content_id , title , summary_html , url , article_page_image_path')->from('article')->where(['content_id' => $contentId , 'status' => 'P'])->get()->row_array();
				}
				if(count($details) > 0){
					switch($type){
						case 'koo' :
							$headers = ['Content-Type:application/json' , 'X-KOO-API-TOKEN:1c7101d3-0d77-457a-90ef-41bb584f1fec'];
							$postFields = ['koo' => strip_tags($details['title']) , 'link' => BASEURL.$details['url'] , 'language' => 'en'];
							$curl = curl_init();
							curl_setopt($curl, CURLOPT_URL, 'https://api.kooapp.com/api/post/koo');
							...
							$return['status2'] = $response;
							if($response['status']=="success"){
								$CI->db->insert('socilamedia_master' , ['content_id' => $contentId , 'type' => $type , 'created_by' => USERID , 'modified_by' => USERID , 'modified_on' => date('Y-m-d H:i:s')]);
								$return['status'] = 1;
								$return['message'] = 'Feed posted successfully';
							}else{
								$return['status'] = 0;
