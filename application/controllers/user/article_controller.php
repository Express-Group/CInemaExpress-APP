		...
		if($content_exist && !empty($last_uri))   
		{
			...
			switch ($sectionname) {
				case ($sectionname == "galleries" || $sectionname == "photos"):
					...
					break;
				case ($sectionname == "video" || $sub_section_name=="e-video"):
					$content_type_id = 4;
					$table           = "video_".$year;
					break;
				case "audios":
		...
		else   // if xml is not created condition will call this
		{
		  ...
		  if($content_id!='' && $page_type=='2')
		  {
				...
				if($content_from=="live" || $content_from=="archive"){
				 if($content_from=="live" && $content_type_id==1){
					 $CI = &get_instance();
					 $this->live_db = $CI->load->database('live_db',TRUE);
					 $review = $this->live_db->query("SELECT movie_name ,movie_director,movie_cast,rating FROM review_master WHERE content_id='".$content_id."' LIMIT 1");
					 if($review->num_rows()==1){
						 $reviewdet = $review->result();
						$data['reviewdetails'] = [$reviewdet[0]->movie_name,$reviewdet[0]->movie_director,$reviewdet[0]->rating,json_decode($reviewdet[0]->movie_cast)]; 
					 }else{
						 $data['reviewdetails'] = ['','','',array()];
					 }
				 }else{
					 $data['reviewdetails'] = ['','','',array()];
				 }
