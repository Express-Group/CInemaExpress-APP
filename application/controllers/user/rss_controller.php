...
class Rss_controller extends CI_Controller {
	...
	public function new_sitemap() {
				...
				$this->live_db->order_by("last_updated_on","desc");
				if($this->input->get("limit")=='20'){
					$this->live_db->limit("20");
				}else{
	...
	public function latest_feed(){
		...
		if($this->input->get('category')!='' && is_numeric($this->input->get('category'))){
			...
			$SectionIds = array(2,6,7,8,9,30,4,15,16,17,18,19);
			$limit = 1;
		}
	...
	public function latest_sitemap(){
		$Response ='';
		$GetArticle = $this->live_db->query("SELECT content_id, title, summary_html, article_page_content_html, publish_start_date, tags, last_updated_on, url, article_page_image_path, article_page_image_alt FROM article WHERE publish_start_date < NOW() AND status='P' ORDER BY last_updated_on DESC LIMIT 10");
		if($GetArticle->num_rows()!=0){
			foreach($GetArticle->result_array() as $ArticleDetails):
				if($ArticleDetails['article_page_image_path']==''){
					...
					$thumbimage = image_url.imagelibrary_image_path.$ArticleDetails['article_page_image_path'];
					$thumbimage = str_replace('original/','w150X150/',$thumbimage);
					$alt=trim($ArticleDetails['article_page_image_alt']);
				}
				$title         = html_entity_decode($ArticleDetails['title'],null,"UTF-8");
				$search        = array('&', '&#39;');
				$replace       = array('&amp;', "'");
				...
				$replace1       = array('&amp;', "'",' ',' ',' ','','','');
				//$content         	= str_replace($search1, $replace1 , $ArticleDetails['article_page_content_html']);
				$content = html_entity_decode($ArticleDetails['article_page_content_html'],ENT_QUOTES,"UTF-8");
				$content = preg_replace('/(<[^>]+) style=".*?"/i', '$1', $content);
				$content = str_replace(['alt=""' , '<p><img' , '/></p>' , '<p></p>' , '<p>&nbsp;</p>' ,'<p>  </p>'] ,['' , '<figure><img' , '/></figure>' , '' ,'' ,''] , $content);
				$EncodedContent 	='<![CDATA[<!doctype html><html><head><link rel="canonical" href="'.BASEURL. html_entity_decode($ArticleDetails['url'],null,"UTF-8").'"/><meta charset="utf-8"/><meta property="op:markup_version" content="v1.0"/><meta property="fb:op-recirculation-ads" content="enable=true placement_id=2362293504088255_2728135694170699"/><meta property="fb:use_automatic_ad_placement" content="enable=true ad_density=default"/><meta property="fb:article_style" content="default"/><meta property="fb:likes_and_comments" content="enable"/></head><body><article><header><figure><img alt="'.$alt.'" src="'.$imagePath.'"/></figure><h1>'.$title.'</h1><time class="op-published" datetime="'.$publish_date_custom->format('Y-m-d\TH:i:s+05:30').'">'.$publish_date_custom->format('Y-m-d\TH:i:s+05:30').'</time><time class="op-modified" datetime="'.$publish_updated_date_custom->format('Y-m-d\TH:i:s+05:30').'">'.$publish_updated_date_custom->format('Y-m-d\TH:i:s+05:30').'</time><address><a href="'.BASEURL. html_entity_decode($ArticleDetails['url'],null,"UTF-8").'">newindianexpress.com</a></address><figure class="op-ad"><iframe width="300" height="250" style="border:0; margin:0;" src="https://www.facebook.com/adnw_request?placement=2362293504088255_2728135694170699&adtype=banner300x250"></iframe></figure></header>'.$content.'<figure class="op-tracker"><iframe src="" hidden><script>';
				...
				$EncodedContent 	.='</iframe></figure>';
				$EncodedContent     .='<figure class="op-tracker"><iframe src="" hidden><script>';
				$EncodedContent 	.='var _comscore = _comscore || [];
										  _comscore.push({ c1: "2",	c2: "16833363", options: {
											...
											</script><noscript><img src="https://sb.scorecardresearch.com/p?c1=2&c2=16833363&cv=3.1&cj=1&comscorekw=fbia"></noscript></iframe></figure>';
				$EncodedContent 	.='<footer><small>Copyright - '.date("Y").' newindianexpress.com. All rights reserved.</small></footer></article></body></html>]]>';
				$Description         	= strip_tags(str_replace($search1, $replace1 , $ArticleDetails['summary_html']));
