...
class Commonwidget extends CI_Controller 
{
	...
	public function captcha()
	{
		...
		$captcha_background = imagecolorallocate($target_layer, 244, 152, 29);
	...
	public function getLivenowContentStatic(){
		$articleId=htmlspecialchars($this->input->post('article_id'));
		$articleurl=htmlspecialchars($this->input->post('article_url'));
		$image_url=htmlspecialchars($this->input->post('image_url'));
		$timestamp=htmlspecialchars($this->input->post('timestamp'));
		$agency_name=htmlspecialchars($this->input->post('agency_name'));
		$content_title=htmlspecialchars($this->input->post('content_title'));
		$image_url=($image_url=='')? image_url.imagelibrary_image_path.'logo/nie_logo_600X390.jpg':htmlspecialchars($this->input->post('image_url'));
			...
			if($Data['status']==1){
				if(strtotime($Data['date']) > $timestamp || $Data['pin']=='1'){
					...
					if(isset($Data['pin']) && $Data['pin']=='1'){
						$pinTemplate .='<div itemtype="https://schema.org/BlogPosting" itemprop="liveBlogUpdate" itemscope="itemscope" style="box-shadow: 0px 2px 6px 2px #0000002e;" class="live-inner-content live-pinned" data-timestamp="'.strtotime($Data['date']).'">'.$liveSchema.$temp.'</div>';
					}else{
	...
	public function GetRelatedArticles(){
		...
		$Result=$this->widget_model->GetRelatedArticles($ContentID,$Contenttype);
		$Template='';
		foreach($Result as $Data):
			if($Contenttype==1){
				$Img=$this->formImage($Data->article_page_image_path);
				$Class="col-md-6";
			}else if($Contenttype==2){
				$Img=$this->formImage($Data->video_image_path);
				$Class="col-md-6";
			}else if($Contenttype==3){
				$Img=$this->formImage($Data->first_image_path);
				$Class="col-md-6";
			}
			$Template .='<div class="'.$Class.'" style="padding-right: 5px;">';
			$Template .='<a  href="'.BASEURL.$Data->url.'"><img src="'.$Img.'" class="img-responsive"></a>';
			$Template .='<div class="screen"></div>';
			$Template .='<h4 style="font-weight: 700;"><a href="'.BASEURL.$Data->url.'">'.$Data->title.'</a></h4>';
			//$Template .='<div class="descript">'.str_replace(['<p>','</p>'],'',$Data->summary_html).'</div>';
			$Template .='</div>';
		endforeach;
		echo $Template;
	}
	public function formImage($ImageFile){
		if($ImageFile!=''){
				$ImageFile=image_url.imagelibrary_image_path.str_replace('original','w600X390',$ImageFile);
		}else{
			$ImageFile=image_url.imagelibrary_image_path.'logo/nie_logo_600X390.jpg';
		} 
		return $ImageFile;
	}
...
class view extends Commonwidget
{
	public function post_comment(){
		...
		if($name!='' && $email!='' && $comment!='' && $content_id!='' && $article_title!='' && $content_type_id!=''){
			$content_type_id = $this->comment_model->insert_article_comment();	
