...
$AuthorDetails=$this->widget_model->GetAuthorDetailsById($AuthorId,3);	
}
...
$TotalRows = $this->live_db->query("SELECT content_id FROM article WHERE status='P' AND publish_start_date < NOW() AND author_name='".$AuthorName."'")->num_rows();
...
//$columnnist_articles_list = $this->widget_model->get_Stories_For_Author('10', $AuthorId, $content['mode'],$AuthorName);
$columnnist_articles_list = $this->live_db->query("SELECT content_id FROM article WHERE status='P' AND publish_start_date < NOW() AND author_name='".$AuthorName."' ORDER BY content_id DESC LIMIT ".$row.", 15")->result_array();
//ATTRIBUTES ENDS..
?>
<div class="row author-panel">
	<div class="col-md-3 col-ls-3 col-xs-12 col-sm-12 text-center">
	<?php
	$Dummy=base_url('images/static_img/author_noimage.png');
	...
	?>
		<img src="<?php print $Dummy;?>" data-src="<?php print $ImagePath; ?>" class="img-responsive author-img-style" style="border-radius: 50%;height: 150px; width: 150px;object-fit:cover;border: 4px solid red;display:inline-block;">
	</div>
	<div class="col-md-9 col-ls-9 col-xs-12 col-sm-12 text-center">
		<h2 class="author-panel-heading" style="font-weight:700;color:#ed1c24;margin:0;"><span><?php print $AuthorDetails[0]->AuthorName; ?></span></h2>
	...
	<div class="col-md-12">
		...
		<p class="bio-hr hidden-sm hidden-xs"></p>
      ...
      <div class="current all_column">
        ...
        <div class="sub_column"> <i class="fa fa-chevron-right"></i>
          <div class="ColumnList">
            <h5 style="font-weight:700;"><a class="article_click" href="<?php echo $live_article_url; ?>" ><?php echo $display_title;?></a></h5>
