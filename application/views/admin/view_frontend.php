...
if($section_name=="authors"){
	...
	if($this->input->get('search_term')==''){
		$searchstr = str_replace('+',' ',$this->input->get('search_term_txt'));
	}
	if($searchstr==''){
		$searchstr = str_replace('_',' ',$this->uri->segment(2));
	}
...
<html lang="en">
  ...
  <script type="application/x-javascript"> addEventListener("load", function() { setTimeout(hideURLbar, 0); }, false); function hideURLbar(){ if (window.scrollY == 0) window.scrollTo(0,1); }; </script>
	...
	<?php for($n=0;$n<count($brs);$n++): ?>
	{
			"@type": "ListItem",
			"position": "<?php echo $n +2; ?>",
			"item": { 
				"@id": "<?php echo BASEURL.$brs[$n]['url'];?>", 
				"name": "<?php echo ucfirst($brs[$n]['name']) ?>" 
			} 
	<?php if($n==count($brs)-1): ?>
	}
	<?php else: ?>
	},
		<?php endif; ?>
