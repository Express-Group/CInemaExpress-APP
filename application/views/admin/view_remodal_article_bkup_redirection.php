...
$pubDate = date_format(date_create($page_det['publish_start_date']),"Y-m-dTH:i:s+08:00");
$LastUpDate = date_format(date_create($page_det['last_updated_on']),"Y-m-dTH:i:s+08:00");
?>
<?php
...
<link rel="<?php if($AMP==1){ echo 'amphtml'; }else{ echo 'canonical'; } ?>" href="<?php echo $Canonicalurl;?>" />
...
<script type="application/ld+json">
{
	...
	"author": {
		"@type": "Person",
		"name": "<?php echo $page_det['author_name']; ?>"
	},
	"publisher": {
		"@type": "Organization",
		"name": "CinemaExpress",
		"logo": {
			"@type": "ImageObject",
			"url": "<?php echo image_url; ?>images/FrontEnd/images/NIE-logo21.png"
		}
	},
	"description": "<?php echo strip_tags($page_det['summary_html']);?>"
}
</script>
<?php if(($section_id== 4|| $parent_section_id==4) && $content_type_id==1): ?>
<script type="application/ld+json">
{
	...
	"reviewBody": "<?php echo strip_tags($content); ?>", 
	"author": {
		"@type": "Person",
		"name": "<?php echo $page_det['author_name']; ?>",
	...
	"itemReviewed": {
		"@type": "Movie",
		"name": "<?php echo $reviewdetails[0]; ?>",
		"datePublished": "<?php echo $pubDate; ?>",
		"image": "<?php echo $image_path;?>",
		"sameAs": "<?php echo base_url() ?>" ,
		"director": {
			"@type": "Person",
			"name": "<?php echo $reviewdetails[1]; ?>" 
		}<?php if(count($reviewdetails[3]) > 0){ ?>,<?php } ?>
		<?php
		if(count($reviewdetails[3]) > 0){
		?>
		"actor":[ 
			<?php for($t=0;$t<count($reviewdetails[3]);$t++): ?>
			<?php if($t==(count($reviewdetails[3])-1)): ?>
			{"@type": "Person", "name": "<?php echo $reviewdetails[3][$t]; ?>"} 
			<?php else:?>
			{"@type": "Person", "name": "<?php echo $reviewdetails[3][$t]; ?>"} ,
			<?php endif; ?>
			<?php endfor; ?>
	...
	"reviewRating": {
		...
		"bestRating": "5", 
		"worstRating": "1", 
		"ratingValue": "<?php echo $reviewdetails[2]; ?>"
	} 
}
</script>
<?php endif; ?>
