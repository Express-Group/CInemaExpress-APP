...

{
	...
	*/
...

	{
		...
		$article_details['review']=$review_value;
		...
		$null_value 		= "NULL";
			...
			$this->db->update('articlemaster',array('review'=>$review_value));
			...
			if($article_id != '' && $article_id != 'NULL') {
				...
				$this->insert_live_article($article_details['LiveArticleDetails'],$review_value);
			...
			*/
			...
			$article_details['content_id'] = $article_id;
				...
				if($txtStatus == 'P') {
					...
					$this->live_db->where('content_id',$article_id);
					$this->live_db->update('article',array('review'=>$review_value));
		...
		foreach($article_manager as $article) {
			...
			$set_status ='<div class="buttonHolder">';
				...
				if(defined("USERACCESS_EDIT".$Menu_id) && constant("USERACCESS_EDIT".$Menu_id) == 1){
					$set_status .= '<a class="button tick tooltip-2"  href="'.base_url().folder_name.'/'.$edit_url.'" target="_blank" title="Edit"><i class="fa fa-pencil"></i></a><a class="button tick tooltip-2"  href="'.BASEURL.$article['url'].'?page=preview" target="_blank" title="Preview"><i class="fa fa-eye"></i></i></a><a class="button tick tooltip-2 share-btn"  data-contentid="'.$article['content_id'].'" data-tbl="" data-href="'.BASEURL.$article['url'].'" target="_blank" title="Share"><i class="fa fa-share-alt"></i></a>';
				}
				else {
					$set_status .= '';
				}
	...
	*/
	...
	public function get_review($article_id){
		$Data=$this->db->query("SELECT review FROM articlemaster WHERE content_id='".$article_id."'")->result();
