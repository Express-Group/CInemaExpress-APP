...
class Widget_model extends CI_Model
{
  ...
  public function __construct() 
	...
	public function AmpStatus($ContentID=''){
		if($ContentID==''){
			return ['status'=>0,'url'=>''];
		}else{
			$Query=$this->live_db->query("SELECT amp_status,url FROM article WHERE content_id='".$ContentID."'");
			if($Query->num_rows()==0){
				return ['status'=>0,'url'=>''];
			}else{
				$result=$Query->result();
				return ['status'=>@$result[0]->amp_status,'url'=>@$result[0]->url];
			}
		}
	}
 ...
 public function get_content_type($content_id)
	...
	//ADDED  TO GET AUTHOR DETAILS BASED ON ID..
	public function GetAuthorDetailsById($authorId,$type){
		$class_object = new Widget_config_class;
		return $class_object->GetAuthorDetailsById($authorId, $type);
	}
	//END...
	
	public function getReviews($article_id=0){
		$class_object = new Widget_config_class;
		return $class_object->getReviews($article_id);
	}
	
	public function GetRelatedArticles($ContentId,$ContentType){
		$class_object = new Widget_config_class;
		return $class_object->GetRelatedArticles($ContentId,$ContentType);
	}

	public function authorTwitter($content_id){
		$Data=$this->db->query("SELECT twitter FROM authormaster WHERE Author_id = (SELECT Author_ID FROM articlerelateddata WHERE content_id ='".$content_id."')");
		if($Data->num_rows()==0){
			return 0;
		}else{
			$Review=$Data->result();
			return $Review[0]->twitter;
		}
	}
	public function getseotags($contentID){
		$this->live_db->select('tag_name');
		$this->live_db->from('seo_tags');
		$this->live_db->where('content_id' , $contentID);
		$this->live_db->limit(1);
		$seotags = $this->live_db->get();
		if($seotags->num_rows() > 0){
			$result = $seotags->row_array();
...
class Widget_config_class extends Widget_model
{
	public function GetRelatedArticles($ContentId,$ContentType){
		if($ContentType==1){
				$TableName='article';
				$ImageName='article_page_image_path';
			}else if($ContentType==3){
				$TableName='gallery';
				$ImageName='first_image_path';
			}else if($ContentType==2){
				$TableName='video';
				$ImageName='video_image_path';
			}
		$SectionID=$this->live_db->query("SELECT section_id FROM ".$TableName." WHERE content_id='".$ContentId."'");
		if($SectionID->num_rows()==0){
			return '';
		}else{
			$Section=$SectionID->result();
			$Section=$Section[0]->section_id;

		$PrevoiusArticle=$this->live_db->query("SELECT title,url,summary_html,".$ImageName." FROM  ".$TableName." WHERE section_id='".$Section."' AND content_id <'".$ContentId."' ORDER BY content_id DESC LIMIT 1 ")->result();
		$NextArticle=$this->live_db->query("SELECT title,url,summary_html,".$ImageName." FROM  ".$TableName." WHERE section_id='".$Section."' AND content_id >'".$ContentId."' ORDER BY content_id  LIMIT 1 ")->result();
		
		if(count($PrevoiusArticle) < 1){
				$NextArticle=$this->live_db->query("SELECT title,url,summary_html,".$ImageName." FROM  ".$TableName." WHERE section_id='".$Section."' AND content_id >'".$ContentId."' AND publish_start_date < NOW() AND status='P' ORDER BY content_id  LIMIT 2 ")->result();
		}
		/* if(count($NextArticle) ==1){
				$PrevoiusArticle=$this->live_db->query("SELECT title,url,summary_html,".$ImageName." FROM  ".$TableName." WHERE section_id='".$Section."' AND content_id <'".$ContentId."' AND publish_start_date < NOW() AND status='P' ORDER BY content_id DESC LIMIT 2 ")->result();
		} */
		if(count($NextArticle) ==0){
				$PrevoiusArticle=$this->live_db->query("SELECT title,url,summary_html,".$ImageName." FROM  ".$TableName." WHERE section_id='".$Section."' AND content_id <'".$ContentId."' AND publish_start_date < NOW() AND status='P' ORDER BY content_id DESC LIMIT 2 ")->result();
		}
		
		$Article=array_merge($PrevoiusArticle,$NextArticle);
	...
	//ADDED  TO GET AUTHOR DETAILS BASED ON ID..
	public function GetAuthorDetailsById($authorId=0,$type){
		$this->load->database();
		if($type==0){
			$Details=$this->db->get_where('authormaster',array('AuthorName'=>$authorId , 'authorType' => 2))->result();
		}else if($type==3){
			$Details=$this->db->get_where('authormaster',array('AuthorName'=>$authorId , 'authorType' => 1))->result();
		}else{
			$Details=$this->db->get_where('authormaster',array('AuthorName'=>$authorId))->result();
		}
		return $Details;
	}
	//END...
	...
	public function getReviews($article_id){
		$Data=$this->live_db->query("SELECT review FROM article WHERE content_id='".$article_id."'");
		if($Data->num_rows()==0){
			return 0;
		}else{
			$Review=$Data->result();
